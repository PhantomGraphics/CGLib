...
void VkTriangleRenderer::create(const VulkanContext& ctx,
                                 ...
                                 uint32_t framesInFlight)
{
    ...
    pCfg.cullMode            = config_.cullBack ? VK_CULL_MODE_BACK_BIT : VK_CULL_MODE_NONE;
    pCfg.depthTest           = true;
    pCfg.depthWrite          = true;
    pCfg.blendEnable         = config_.blendEnable;
    pCfg.samples             = config_.samples;

    pipeline_.create(ctx, renderPass, pCfg);
}

void VkTriangleRenderer::destroy(VkDevice device)
{
...
void VkTriangleRenderer::upload(const VulkanContext& ctx,
                                 const VulkanCommandPool& pool,
                                 const Buffer& buffer)
{
...
void VkTriangleRenderer::updateMVP(uint32_t frameIndex, const glm::mat4& mvp)
{
...
void VkTriangleRenderer::render(VkCommandBuffer cmd, uint32_t frameIndex)
{
