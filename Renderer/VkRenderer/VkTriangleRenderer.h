...
/// @brief Vulkan renderer for triangle polygons (corresponds to OpenGL TriangleRenderer).
///
/// Vertex data is passed through two separate bindings:
///   binding 0 - position (vec3, stride=12)
///   binding 1 - color    (vec4, stride=16)
///
/// Draws indexed triangles using VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST.
class VkTriangleRenderer : public IVkRenderer {
public:
    struct Config {
        std::vector<uint32_t> vertSpv;                          ///< SPIR-V for triangle.vert.
        std::vector<uint32_t> fragSpv;                          ///< SPIR-V for triangle.frag.
        bool blendEnable = false;                               ///< Enable alpha blending.
        bool cullBack    = true;                                ///< Enable back-face culling.
        VkSampleCountFlagBits samples = VK_SAMPLE_COUNT_1_BIT; ///< MSAA sample count (must match render pass).
    };

    using Buffer = VkTriangleBufferData;

    explicit VkTriangleRenderer(Config config) : config_(std::move(config)) {}
