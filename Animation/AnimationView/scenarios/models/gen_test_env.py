...
CGLib/GltfViewer/scenarios/models/gen_test_env.py and CGApp/Universe/scenarios/models/
gen_test_env.py (same fixture, kept as a separate per-app copy per this repo's convention of not
sharing scenario test assets across modules). Hue sweeps around the horizon and brightness fades
from a bright zenith to a dim nadir, so LoadEnvironmentHDR's direction-to-UV mapping is visibly
exercised (a flat/solid environment couldn't tell a working equirect-to-cube conversion apart
from a bug that just samples the same texel everywhere)."""
import math
import os
