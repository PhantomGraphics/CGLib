...
# AnimationView.exe is built from the CMake source dir below. Phantom and CGLib each have their own
...
for ($root = $srcDir; $root; $root = Split-Path -Parent $root) {
    ...
    $candidate = (@($root, "build", $preset, $rel, "AnimationView.exe") | Where-Object { $_ }) -join '\'
    $tried    += $candidate
