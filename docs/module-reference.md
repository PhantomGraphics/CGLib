...
各モジュールのクラス構成・API・設計方針をまとめた詳細ドキュメント。
プロジェクト全体の概要・ビルド方法は [`../README.md`](../README.md) を参照。

Vulkan ベースのレンダリング基盤と、数学・画像処理・ImGui UI・シーングラフ・ファイル I/O・空間データ構造・
ボリューム処理を提供するスタティックライブラリ群。

> 各モジュールはかつて独立したリポジトリとして開発され、後に本リポジトリへフラットに
> 統合された。以下、「Math〜UIWidgets」が中核 4 モジュール、
> 「Scene〜GltfViewer」がその上に載る上位モジュール。
