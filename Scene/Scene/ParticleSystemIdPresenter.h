...
/// @brief Presenter that renders a ParticleSystemScene using per-particle ID colors.
///
/// Encodes each particle's scene ID as a unique color for GPU-side picking.
...
public:
	...
	ParticleSystemIdPresenter(Scene::ParticleSystemScene* psScene, Phantom::Renderer::PointRenderer* renderer) :
		model(psScene),
		view(renderer)
	{
	}

	/// @brief Build CPU-side vertex data (positions, ID-encoded colors, sizes).
	void build() override;
