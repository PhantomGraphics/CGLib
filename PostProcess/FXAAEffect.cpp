...
void FXAAEffect::init(const PostEffectContext& ctx, VkImageView inputView, VkSampler inputSampler)
{
    ctx_ = ctx.ctx;
...
void FXAAEffect::apply(VkCommandBuffer cmd, uint32_t frameIndex)
{
    VkExtent2D extent = output_.getExtent();
    UBO ubo{};
    ubo.texelSize         = glm::vec2(1.0f / static_cast<float>(extent.width),
                                       1.0f / static_cast<float>(extent.height));
    ubo.contrastThreshold = params_.contrastThreshold;
    ubo.relativeThreshold = params_.relativeThreshold;
    ubo.subpixelBlend     = params_.subpixelBlend;
...
void FXAAEffect::destroy(VkDevice device)
{
