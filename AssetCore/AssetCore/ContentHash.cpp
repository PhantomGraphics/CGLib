...
public:
    ...
    void update(const std::uint8_t* data, std::size_t size)
    {
        ...
        while (size != 0) {
            ...
            std::copy(data, data + take, block_.begin() + static_cast<std::ptrdiff_t>(used_));
            used_ += take;
            data += take;
            size -= take;
    ...
    std::array<std::uint8_t, 32> finish()
    {
        ...
        for (int i = 0; i < 8; ++i)
            block_[56 + i] = static_cast<std::uint8_t>(bits >> (56 - i * 8));
        transform(block_.data());
...
std::string toHex(const std::array<std::uint8_t, 32>& bytes)
{
    ...
    std::string result;
    result.reserve(7 + bytes.size() * 2);
    result = "sha256:";
    for (const auto byte : bytes) { result.push_back(hex[byte >> 4]); result.push_back(hex[byte & 0xf]); }
    return result;
}

} // namespace

ContentHash ContentHash::fromBytes(const void* data, std::size_t size)
{
    Sha256 hash;
    hash.update(static_cast<const std::uint8_t*>(data), size);
    return ContentHash(toHex(hash.finish()));
}

std::optional<ContentHash> ContentHash::fromFile(const std::filesystem::path& path)
{
    std::error_code ec;
    if (!std::filesystem::is_regular_file(path, ec) || ec)
        return std::nullopt;
    // Pass the native path as a string after the explicit regular-file check. This avoids the
    // old MSVC STL path-stream overload recursively normalizing a missing path under some
    // redirected temporary directories.
