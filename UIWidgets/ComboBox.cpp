...
{
	if (indexGetter_) selectedIndex = indexGetter_();
	...
	if (ImGui::BeginCombo(name.c_str(), preview)) {
		for (int i = 0; i < static_cast<int>(items.size()); ++i) {
			...
			if (ImGui::Selectable(items[i].c_str(), is_selected) && i != selectedIndex) {
				selectedIndex = i;
				if (indexSetter_) indexSetter_(i);
				if (onChange_) onChange_(i);
			}
			if (is_selected)
