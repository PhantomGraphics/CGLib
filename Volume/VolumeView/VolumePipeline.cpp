...
void VolumePipeline::create(const Phantom::VKG::VulkanContext& ctx, VkRenderPass renderPass, uint32_t framesInFlight,
    ...
    std::vector<VkVertexInputBindingDescription> bindings = {
        {0, sizeof(PointVertex), VK_VERTEX_INPUT_RATE_VERTEX},
    };
    std::vector<VkVertexInputAttributeDescription> attrs = {
        {0, 0, VK_FORMAT_R32G32B32_SFLOAT, static_cast<uint32_t>(offsetof(PointVertex, pos))},
        {1, 0, VK_FORMAT_R32G32B32A32_SFLOAT, static_cast<uint32_t>(offsetof(PointVertex, color))},
    };
...
void VolumePipeline::destroy(VkDevice device) {
    for (auto& ubo : ubos_) {
        ubo.destroy(device);
    }
    ubos_.clear();
...
void VolumePipeline::updateUBO(uint32_t frameIndex, const UBO& ubo) {
