# run_volume_scenarios.ps1 - Run all VolumeView scenario tests
# Usage: .\CGLib\Volume\VolumeView\run_volume_scenarios.ps1 [-Configuration Debug|Release]
# Run from the repository root or the CGLib\Volume\VolumeView directory.
...
# VolumeView.exe is built from the CMake source dir below. Phantom and CGLib each have their own
...
for ($root = $srcDir; $root; $root = Split-Path -Parent $root) {
    ...
    $candidate = (@($root, "build", $preset, $rel, "VolumeView.exe") | Where-Object { $_ }) -join '\'
    $tried    += $candidate
